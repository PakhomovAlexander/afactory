...
fn auxiliary_usage_is_charged_once_even_when_its_model_refuses_output() {
    let result = account(Some(&response()), Some(OPUS));
    assert_eq!(
        result.error,
...
fn selected_entry_or_whole_map_can_explain_top_level_without_double_counting() {
    ...
    let result = account(Some(&value), Some(OPUS));
...
fn only_explicit_native_identity_mapping_is_accepted() {
    ...
    assert!(
        account(Some(&value), Some(OPUS)).error.is_some(),
        "no date stripping"
    );
    value["modelUsage"]["claude-opus-5-20260101"]["canonicalModel"] = json!(OPUS);
    assert!(account(Some(&value), Some(OPUS)).error.is_none());
    assert!(
        account(Some(&value), None).error.is_none(),
        "no explicit model restriction"
    );
    assert!(
        account(Some(&value), Some("opus")).error.is_some(),
...
#[test]
fn no_model_restriction_keeps_complete_multi_model_accounting() {
    let result = account(Some(&response()), None);
    assert_eq!(result.usage.unwrap().chargeable_tokens.get(), 242_845);
    assert!(result.observation.is_none());
    assert!(result.error.is_none());
}
