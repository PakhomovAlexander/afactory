//! `af self` and dispatch against a directory release source: installs verified by the release
//! checksums or their signature, pins that bind bytes, the self-management floor, and the
//! versions `remove` and `prune` must keep.
...
fn self_install_update_rollback_and_remove_against_a_directory_source() {
    ...
    let install = sandbox
        .command(Path::new(AF))
        .args(["self", "install", "0.7.1"])
        .output()
        .unwrap();
    assert!(install.status.success(), "{}", err(&install));
    assert!(sandbox.versions().join("0.7.1/af").is_file());
    let receipt = std::fs::read_to_string(sandbox.versions().join("0.7.1/receipt.toml")).unwrap();
    assert!(
    ...
    assert_eq!(sandbox.default_target().as_deref(), Some("0.7.1"));

    let remove_default = sandbox
        .command(&real)
        .args(["self", "remove", "0.7.1"])
        .output()
        .unwrap();
...
#[test]
fn nothing_older_than_the_first_self_managing_release_is_activated_or_dispatched_to() {
    let sandbox = Sandbox::new();
    sandbox.publish("0.7.0", false);
    for args in [
        &["self", "install", "0.7.0"][..],
        &["self", "update", "--version", "0.7.0"],
    ] {
        let refused = sandbox.command(Path::new(AF)).args(args).output().unwrap();
        assert!(!refused.status.success(), "{args:?}");
        assert!(
            err(&refused).contains("predates self-management"),
            "{args:?}: {}",
            err(&refused)
        );
        assert!(!sandbox.versions().join("0.7.0").exists());
    }

    // A lock below the floor is an older pin this binary cannot honour: it runs and says so.
    let repo = sandbox.pinned_repo("repo", &Sandbox::lock("0.7.0", None));
    let run = sandbox
        .command(Path::new(AF))
    ...
    assert!(
        err(&run).contains("predates self-management") && err(&run).contains("--af <version>"),
        "{}",
        err(&run)
    );
    ...
    let explicit = sandbox
        .command(Path::new(AF))
        .args(["review", "campaigns"])
        .env("AF_VERSION", "0.7.0")
        .output()
        .unwrap();
    assert_eq!(explicit.status.code(), Some(1));
    assert!(
        err(&explicit).contains("predates self-management"),
        "{}",
        err(&explicit)
    );
}
...
fn dispatch_runs_the_version_a_project_pins_when_the_bytes_match() {
    let sandbox = Sandbox::new();
    let digest = sandbox.publish("0.7.1", false);
    let repo = sandbox.pinned_repo("repo", &Sandbox::lock("0.7.1", Some(&digest)));
    ...
    let offline = sandbox
        ...
        .unwrap();
    assert!(
        err(&offline).contains("af self install 0.7.1"),
        "{}",
        err(&offline)
    );
    ...
    assert!(text.contains("fake af 0.7.1 review plan --repo"), "{text}");
    ...
    .unwrap();
    assert_eq!(status["pinned_projects"][0]["version"], "0.7.1");
    write(
    ...
    assert!(
        sandbox.versions().join("0.7.1/af").is_file(),
        "prune kept the pin"
    );
    ...
    let forced = sandbox
        ...
        .unwrap();
    assert!(
        out(&forced).contains("fake af 0.7.1 review plan"),
        "{}",
        out(&forced)
    );
    let onboard = sandbox
        .command(Path::new(AF))
        .args(["onboard", "--refresh-lock", "--af", "0.7.1"])
        .current_dir(sandbox.path("home"))
        .output()
        .unwrap();
    assert!(
        out(&onboard).contains("fake af 0.7.1 onboard --refresh-lock --af 0.7.1"),
        "{}",
        out(&onboard)
    );
...
fn a_pin_binds_bytes_not_just_a_version() {
    ...
    assert!(
        err(&older).contains("no digest for")
            && err(&older).contains("--refresh-lock --af 0.7.1")
            && err(&older).contains("af self install 0.7.1"),
        "{}",
        err(&older)
    );
    ...
    let mismatched = sandbox.pinned_repo("mismatch", &Sandbox::lock("0.7.1", Some(&wrong)));
    let refused = sandbox
        .command(Path::new(AF))
        .args(["review", "campaigns"])
    ...
    let dispatched = sandbox
        ...
        .unwrap();
    assert!(
        out(&dispatched).contains("fake af 0.7.1 review campaigns"),
        "{}",
        out(&dispatched)
    );
...
#[test]
fn releases_since_0_8_0_must_carry_a_valid_signature_when_the_build_has_a_key() {
    let keys = tempfile::tempdir().unwrap();
    let signer = Signer::new(keys.path());
    ...
    // Releases before 0.8.0 predate signing and install from their checksums.
    let legacy = sandbox
        .command(Path::new(AF))
        .args(["self", "install", "0.7.1"])
        .output()
        .unwrap();
    assert!(legacy.status.success(), "{}", err(&legacy));
    let receipt = std::fs::read_to_string(sandbox.versions().join("0.7.1/receipt.toml")).unwrap();
    assert!(
        receipt.contains("verified_by = \"sha256sums\""),
        "{receipt}"
...
fn project_config_cannot_steer_dispatch_and_unavailable_newer_pins_fail_closed() {
    ...
    let older = sandbox
        ...
        .unwrap();
    assert!(
        !sandbox.versions().join("0.7.1").exists(),
...
fn a_bare_binary_without_a_receipt_is_not_installed() {
    let sandbox = Sandbox::new();
    let digest = sandbox.publish("0.7.1", false);
    let dir = sandbox.versions().join("0.7.1");
    ...
    assert!(receipt.contains("version = \"0.7.1\""), "{receipt}");
