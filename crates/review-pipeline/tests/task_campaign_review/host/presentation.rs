...
fn presentation_retains_original_review_ports_and_selected_transport_fields_without_writes() {
    ...
    let reply = serde_json::json!({"findings":[{"severity":"major","file":".af/pipelines/review.toml","line":1,"title":"Missing required behavior","body":"The required behavior is absent","fix":"Implement the missing behavior","confidence":0.9}],"benchmark_demands":[],"dispositions":[]});
    ...
    let selected = events
        ...
        .unwrap();
    let selection: review_core::task::campaign_review::TaskReviewResultSelectedV1 =
        serde_json::from_value(selected.payload.clone()).unwrap();
    let provenance: review_core::task::campaign_review::TaskReviewAttemptProvenanceV2 =
    ...
    let reopened =
        CampaignReviewTaskHost::new(&cas, shared.clone(), &compiler, lease, BTreeMap::new())
            .unwrap();
