    ...
    fn invoke(
        ...
        let (message, usage) = if n == 0 {
            ...
            if self.retry && n == 1 {
                ...
                (
                    r#"{"findings":[],"benchmark_demands":[],"dispositions":[]}"#,
                    11,
                )
            }
        };
    ...
    for (provider_kind, admitted, retry, wide) in [
        ...
        for pass in 0..if provider_only { 4 } else { 2 } {
            let models = plan
                ...
                .collect();
            let host =
                CampaignReviewTaskHost::new(&cas, shared.clone(), &compiler, lease.clone(), models)
                    .unwrap();
            let authority =
...
fn review_report_binds_wide_task_charge_and_freezes_its_accounting_prefix() {
    ...
    let mut store = EventStore::open(&path).unwrap();
    {
        let shared = SharedEventStore::new(&mut store);
        let host =
            CampaignReviewTaskHost::new(&cas, shared, &compiler, lease.clone(), models()).unwrap();
