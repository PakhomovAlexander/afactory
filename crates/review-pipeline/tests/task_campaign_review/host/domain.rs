...
pub(super) fn admit_source(
    ...
    reviewers: &[&str],
    heavy: bool,
) -> (CampaignReviewPlanCompiler, TaskLease) {
...
pub(super) fn hosted<'s, 'c, T>(
    ...
    compiler: &'c CampaignReviewPlanCompiler,
    lease: &TaskLease,
    configure: impl FnOnce(CampaignReviewTaskHost<'s, 'c>) -> CampaignReviewTaskHost<'s, 'c>,
    body: impl FnOnce(&CampaignReviewTaskHost<'s, 'c>, &TaskRuntime<'s, '_>, RunReport) -> T,
) -> T {
    let host = configure(
        CampaignReviewTaskHost::new(
            cas,
            shared.clone(),
            compiler,
            lease.clone(),
...
id = "evidence"
kind = "gather"
inputs = [{ name = "decision", type = "review.kernel/GateDecision@1", cardinality = "one", optional = false, snapshot_affinity = "any" }]
outputs = [{ name = "reports", type = "review.kernel/ReportSet@1", cardinality = "one", optional = false, snapshot_affinity = "any" }]
[[nodes]]
id = "collect"
kind = "gather"
