...
use super::*;
use review_core::task::campaign_review::{TaskReviewContextV1, TaskReviewResultSelectedV1};
use review_core::{
...
fn continue_on_head(
    ...
    host: &CampaignReviewTaskHost<'_, '_>,
    lease: &TaskLease,
    head: &BTreeMap<String, Vec<u8>>,
) -> (CampaignReviewPlanCompiler, String) {
    ...
    let successor = CampaignReviewPlanCompiler::reopen(
        cas,
        CapturedCampaignReviewRound::load(cas, &store, "review", &next).unwrap(),
    ...
    let handoff_id =
        review_store::store::task::review_handoff::capture_task_review_handoff(cas, &handoff)
            .unwrap();
    let authority = CapturedTaskAuthority::for_campaign_review(&successor, host, &NoTaskDeveloper);
    store
...
fn rooted<'s, 'c>(
    root: &std::path::Path,
) -> impl FnOnce(CampaignReviewTaskHost<'s, 'c>) -> CampaignReviewTaskHost<'s, 'c> + '_ {
