...
fn captured_command_review_uses_common_attempt_selection_and_replays_canonical_outputs() {
    ...
    let plan_id = plan::artifact(&cas, review_core::task::EXECUTION_PLAN_V1, &compiled);
    let admission = CapturedTaskAuthority::for_campaign_review(
        &compiler,
        &plan::RefuseExecution,
        &NoTaskDeveloper,
    );
...
fn completed_review_retains_blocking_findings_and_required_demands() {
    for demand in [false, true] {
        ...
        let mut store = EventStore::open(directory.path().join("events.sqlite")).unwrap();
        let mut returned =
            serde_json::json!({"findings":[],"benchmark_demands":[],"dispositions":[]});
        if demand {
    ...
    for available in [true, false] {
        ...
        if !available {
            for change in ["omit", "substitute"] {
                ...
                let fork_host = CampaignReviewTaskHost::new(
                    &cas,
                    fork_shared.clone(),
                    &compiler,
                    lease.clone(),
...
fn a_stale_task_writer_cannot_publish_a_canonical_review_conclusion() {
    ...
    let (compiler, lease) = admit(&cas, &mut store, &command_pipeline());
    let shared = SharedEventStore::new(&mut store);
    let old = CampaignReviewTaskHost::new(
        &cas,
        shared.clone(),
        &compiler,
        lease.clone(),
    ...
    .unwrap();
    let authority = CapturedTaskAuthority::for_campaign_review(&compiler, &old, &NoTaskDeveloper);
    let runtime =
    ...
    let fresh = CampaignReviewTaskHost::new(
        &cas,
        shared.clone(),
        &compiler,
        fresh_lease,
