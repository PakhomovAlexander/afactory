use super::*;
use review_pipeline::task::campaign_review::plan::REVIEW_TASK_POLICY_V4;
...
fn integration_is_captured_only_for_heavy_rounds_with_its_original_dormant_allowance() {
    for mode in ["light", "heavy"] {
        ...
        let captured = CapturedCampaignReviewRound::load(&cas, &store, "review", &round).unwrap();
        let compiler =
            CampaignReviewPlanCompiler::capture(&cas, captured, engine, settings_for(mode))
                .unwrap();
