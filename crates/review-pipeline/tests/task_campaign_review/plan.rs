...
fn captured_review_plan_admits_reopens_and_refuses_changed_or_missing_authority() {
    ...
    let authority =
        CapturedTaskAuthority::for_campaign_review(&compiler, &RefuseExecution, &NoTaskDeveloper);
    let lease = store
    ...
    let reopened = CampaignReviewPlanCompiler::reopen(
        &cas,
        CapturedCampaignReviewRound::load_recorded(&cas, &store, "review", &round).unwrap(),
        &engine,
        compiler.policy_id(),
    )
    .unwrap();
...
fn captured_native_runner_requires_exact_model_binding_and_common_provider_budget() {
    ...
    let capture = |settings| {
        CampaignReviewPlanCompiler::capture(
            &cas,
            CapturedCampaignReviewRound::load(&cas, &store, "review", &round).unwrap(),
            engine.clone(),
            settings,
...
fn assert_plan_schemas(cas: &Cas, compiler: &CampaignReviewPlanCompiler, plan: &ExecutionPlanV1) {
