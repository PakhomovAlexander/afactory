//! Installed Campaign Review compilation. Persisted mappings describe exact captured
...
use review_core::task::campaign_review::TASK_REVIEW_RESULT_METADATA_V1;
pub use review_core::task::campaign_review::{
    CAMPAIGN_REVIEW_GATE_OUTCOME_V1 as REVIEW_GATE_OUTCOME_V1,
    CAMPAIGN_REVIEW_ROUND_V1 as REVIEW_ROUND_V1,
};
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct CampaignReviewCompilation {
...
impl ReviewNodeMapping {
    /// Reconstruct the exact Review fan-in after the Task scheduler selected every lane.
...
/// Compile a validated Review definition into the same scheduler and Attempt budget used by
...
pub fn compile_campaign_review(
    loaded: &Loaded,
    context: ReviewCompileContext,
) -> Result<CampaignReviewCompilation, String> {
    ...
    for (index, (name, node)) in planned.nodes.iter().enumerate() {
        ...
        let operation = match node.kind {
            ...
            NodeKind::Task => {
                return Err("Campaign Review cannot declare Task execution nodes".into());
            }
        };
        graph.nodes.insert(
            task_node,
