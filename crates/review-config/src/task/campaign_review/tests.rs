...
fn static_review_has_public_contract_typed_lanes_transitive_gates_and_one_budget() {
    let loaded = crate::Definition::from_toml(PIPELINE)
        ...
        .load()
        .unwrap();
    let compilation = compile_campaign_review(&loaded, context(&loaded)).unwrap();
    ...
    assert_eq!(
        compilation,
        compile_campaign_review(&loaded, context(&loaded)).unwrap()
    );
    ...
    assert_eq!(
        serde_json::from_value::<CampaignReviewCompilation>(wire).unwrap(),
        compilation
    );
}
...
fn codecs_follow_the_declared_contract_and_generation_stays_strict() {
    ...
    for (output, retyped_output) in [
        ...
        let loaded = crate::Definition::from_toml(&retyped)
            ...
            .load()
            .unwrap();
        let error = compile_campaign_review(&loaded, context(&loaded)).unwrap_err();
    ...
    assert!(compile_campaign_review(&loaded, bad).is_err());
    let mut bad = context(&loaded);
...
fn dynamic_review_retains_enveloped_history_and_shared_provider_guards() {
    ...
    let mut compilation = compile_campaign_review(&loaded, config).unwrap();
...
fn captured_review_resources_preserve_node_retry_and_round_caps_on_the_task_ledger() {
    ...
    let mut compilation = compile_campaign_review(&loaded, context).unwrap();
...
#[test]
fn uncapped_review_requires_new_bounded_policy_and_never_invents_a_cap() {
    use super::resources::{ReviewResourcePolicy, review_token_scopes};
    let loaded = crate::Definition::from_toml(PIPELINE)
        .unwrap()
        .load()
    ...
    let compilation = compile_campaign_review(&loaded, context).unwrap();
    assert!(
...
fn owned_review_capture_preserves_static_plan_and_inherits_only_captured_child_authority() {
    ...
    .unwrap();
    let limits = context.limits.clone();
    let frozen = compile_campaign_review(&loaded, context).unwrap();
    assert!(
