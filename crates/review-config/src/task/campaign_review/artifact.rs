//! Explicit conversion at the Campaign Review boundary. Never infer an artifact's encoding
//! from JSON keys: a flat payload can itself contain `type`, `producer`, or `payload`.
...
impl ReviewArtifactCodec {
    ...
    /// Lift one exact Review artifact into a Task port. Existing envelopes keep their own
    ...
    pub fn capture(
        ...
        match self {
            Self::Envelope { .. } => {
                self.read(cas, review_id)?;
                Ok(review_id.into())
            }
            Self::Flat { artifact_type } => {
                let payload = cas.get_json(review_id).map_err(|e| e.to_string())?;
                if !payload.is_object()
                    || content_id(&payload).map_err(|e| e.to_string())? != review_id
                {
    ...
    /// Restore the original CAS reference for a Review operation. This checks both the
