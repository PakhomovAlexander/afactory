//! Typed Campaign Review receipts preserve canonical Review artifacts beside Task output
//! ports. They carry evidence identities, never dispatch or selected-result authority.
//!
//! The `af/LegacyReview*` type strings are the persisted spelling and never change; the Rust
...
pub const CAMPAIGN_REVIEW_ROUND_V1: &str = "af/LegacyReviewRound@1";
pub const CAMPAIGN_REVIEW_GATE_OUTCOME_V1: &str = "af/LegacyReviewGateOutcome@1";
...
/// Input binding for the installed Campaign Review frontend. Unlike TaskReviewRound@1 (a completed
/// canonical reduction), this names one exact Campaign Round awaiting execution.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct CampaignReviewRoundV1 {
...
impl CampaignReviewRoundV1 {
    ...
    pub fn validate(&self) -> Result<(), String> {
        require(
            ...
            "Campaign Review Round needs a bounded Campaign identity",
        )?;
        require(
            event_id(&self.round_event_id) && self.round > 0 && self.epoch > 0,
            "Campaign Review Round needs an exact event, Round and epoch",
        )?;
        require(
            self.artifact_refs().into_iter().all(is_digest),
            "Campaign Review Round needs exact captured authority and Subject identities",
        )
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct CampaignReviewGateOutcomeV1 {
    pub round_event_id: String,
    pub review_node: String,
...
impl CampaignReviewGateOutcomeV1 {
    pub fn validate(&self) -> Result<(), String> {
        ...
        require(
            ...
            "Campaign Review Gate outcome needs exact Round, node and decision identities",
        )?;
        require(
            matches!(
                self.outcome,
...
/// Exact context captured by the Campaign Review host after the common Attempt reservation.
