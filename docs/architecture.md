...
fixtures/
  ...
  synthetic/     16 cases generated by running the legacy harness
  adversarial/   four cases specified where no legacy behavior exists to capture
  consumers/     the policies downstream repositories pin, planned with every release binary
...
`crates/review-core/tests/legacy_corpus.rs` runs the contracts against real reviewer output. A contract that cannot
...
- **`fixtures/legacy/`** — frozen bundles of real reviewer output. Every stage output must parse
  ...
  satisfy the I-JSON numeric domain. This is per-project review data and stays in the project
  that captured it. Those tests are `#[ignore]`d, so cargo reports them as `ignored` rather
  than as passing: a runtime skip would print `ok`, and cargo hides the explanation. A
  checkout that holds a corpus runs them explicitly with `--ignored`, where a missing corpus is
  a failure.
...
hides the notice that says why — so a checkout with no such corpus would have shown a green
...
`crates/review-store/tests/replay_synthetic.rs` replays all 13 ledger cases by **parsing their own transcripts** and
...
`crates/review-store/tests/legacy_ledgers.rs` imports every frozen ledger under `fixtures/legacy/` and every ledger
...
exercise the first branch; the synthetic ledgers exercise both. `crates/review-store/tests/crash_replay.rs` kills the process at each boundary: the projection after
