...
|---|---|---|
| Actual light command path | `reviewctl/src/self_optimizer.rs` selects the light package only for a candidate-kind catalog with captured policy; `review-pipeline/src/task/optimization.rs` profiles and validates the generated proposal before existing M2 preparation | `reviewctl/tests/self_optimizer.rs::light_strategy_generates_one_candidate_without_exposing_source_to_author_workers` runs command, signed decision, protected arms, evaluator and replay |
