...
| Closed Review findings | Economics production/gating, candidate-binding refusal, enforced recipe declarations, token-only ordinary payback, adoption receipt/observation production, the shared 64-edit bound, light/report request-digest separation and schema/Rust control-character parity are mapped above or in the named core/schema tests. | `af/tests/self_optimizer.rs`, `af/src/self_optimizer.rs::light_and_report_requests_for_one_capture_have_distinct_task_ids`, `review-core/tests/optimization_light.rs`, `review-core/tests/schema_parity.rs`, and `review-store/src/store/task/delivery.rs` |
...
cache setup retains lookup/materialization timing, source digest and bytes made available.
